...
// =============================================================================
// Error path tests for SslContext
// =============================================================================

TEST(SslContextTestSuite, SetCipherListBadStringThrows)
{
    SslContext ctx(TLS_server_method());
    // A string with only an unknown cipher name causes SSL_CTX_set_cipher_list to return 0.
    EXPECT_THROW(ctx.SetCipherList(std::string("INVALID-CIPHER-NAME-THAT-DOES-NOT-EXIST")), SslException);
}

TEST(SslContextTestSuite, SetCurvesListBadStringThrows)
{
    SslContext ctx(TLS_server_method());
    EXPECT_THROW(ctx.SetCurvesList("not-a-valid-curve"), SslException);
}

TEST(SslContextTestSuite, UsePrivateKeyFileMissingThrows)
{
    SslContext ctx(TLS_server_method());
    EXPECT_THROW(ctx.UsePrivateKeyFile("nonexistent_key.pem"), SslException);
}

TEST(SslContextTestSuite, UseCertificateChainFileMissingThrows)
{
    SslContext ctx(TLS_server_method());
    EXPECT_THROW(ctx.UseCertificateChainFile("nonexistent_chain.pem"), SslException);
}

TEST(SslContextTestSuite, LoadVerifyFileMissingThrows)
{
    SslContext ctx(TLS_server_method());
    EXPECT_THROW(ctx.LoadVerifyFile("nonexistent_ca.pem"), SslException);
}

TEST(SslContextTestSuite, LoadVerifyPemInvalidThrows)
{
    SslContext ctx(TLS_server_method());
    EXPECT_THROW(ctx.LoadVerifyPem("not valid pem"), SslException);
}

TEST(SslContextTestSuite, SetMinProtoVersionNoThrow)
{
    SslContext ctx(TLS_server_method());
    EXPECT_NO_THROW(ctx.SetMinProtoVersion(TLS1_2_VERSION));
}

TEST(SslContextTestSuite, SetMaxProtoVersionNoThrow)
{
    SslContext ctx(TLS_server_method());
    EXPECT_NO_THROW(ctx.SetMaxProtoVersion(TLS1_3_VERSION));
}

TEST(SslContextTestSuite, SetVerifyModeNoThrow)
{
    SslContext ctx(TLS_server_method());
