...
TEST(HelpSslX509, verify_chain_self_signed)
{
    std::ifstream file("cert.pem");
    ASSERT_TRUE(file.good());
    auto cert = SslX509(std::move(file));
    ...
    EXPECT_TRUE(SslX509::VerifyChain(cert, {}, store.Get()));
}

TEST(HelpSslX509, verify_chain_null_store_returns_false)
{
    std::ifstream file("cert.pem");
    ASSERT_TRUE(file.good());
    auto cert = SslX509(std::move(file));

    EXPECT_FALSE(SslX509::VerifyChain(cert, {}, nullptr));
}

TEST(HelpSslX509, verify_chain_untrusted_returns_false)
{
    std::ifstream file("cert.pem");
    ASSERT_TRUE(file.good());
    auto cert = SslX509(std::move(file));

    // Empty store — cert is not trusted.
    SslX509Store store;
    EXPECT_FALSE(SslX509::VerifyChain(cert, {}, store.Get()));
}

TEST(HelpSslX509, borrow_shares_pointer)
{
    std::ifstream file("cert.pem");
    ASSERT_TRUE(file.good());
    auto cert = SslX509(std::move(file));
    ...
    X509 *raw = cert.Get();
    auto borrowed = SslX509::Borrow(raw);

    // Borrow bumps the refcount but returns the same underlying object.
    EXPECT_EQ(borrowed.Get(), raw);
    EXPECT_NE(borrowed.Get(), nullptr);
}

TEST(HelpSslX509, load_chain_from_file)
{
    auto chain = SslX509::LoadChainFromFile("cert.pem");
    EXPECT_EQ(chain.size(), 1u);
    EXPECT_NE(chain[0].Get(), nullptr);
}

TEST(HelpSslX509, load_chain_from_file_missing_throws)
{
    EXPECT_THROW(SslX509::LoadChainFromFile("nonexistent_chain.pem"), SslException);
}

TEST(HelpSslX509, pem_read_from_stream)
{
    std::ifstream file1("cert.pem");
    ASSERT_TRUE(file1.good());
    auto cert = SslX509(std::move(file1));
    X509 *original_ptr = cert.Get();

    // PemRead replaces the underlying object.
    std::ifstream file2("cert.pem");
    ASSERT_TRUE(file2.good());
    cert.PemRead(std::move(file2));

    // The pointer changes because PemRead assigns a newly-parsed cert.
    EXPECT_NE(cert.Get(), nullptr);
    // Both certs carry the same CN.
    EXPECT_EQ(cert.GetCommonName(), "test.example.com");
    (void)original_ptr;
}

TEST(HelpSslX509, get_fingerprint_sha1)
{
    std::ifstream file("cert.pem");
    ASSERT_TRUE(file.good());
    auto cert = SslX509(std::move(file));

    std::string fp = cert.GetFingerprint(EVP_sha1());
    // SHA-1 produces 20 bytes = 40 hex chars.
    EXPECT_EQ(fp.length(), 40u);
    for (char c : fp)
        EXPECT_TRUE((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f'));
}

TEST(HelpSslX509, get_extension_value_present)
{
    std::ifstream file("cert.pem");
    ASSERT_TRUE(file.good());
    auto cert = SslX509(std::move(file));

    // NID_subject_key_identifier is present in the test cert.
    auto val = cert.GetExtensionValue(NID_subject_key_identifier);
    EXPECT_TRUE(val.has_value());
}

TEST(HelpSslX509, get_extension_value_absent_returns_nullopt)
{
    // Default-constructed cert has no extensions.
    auto cert = SslX509();
    auto val = cert.GetExtensionValue(NID_subject_key_identifier);
    EXPECT_FALSE(val.has_value());
}

TEST(HelpSslX509, get_path_length_constraint_non_ca)
{
    // A default-constructed cert has no basic constraints extension.
    auto cert = SslX509();
    EXPECT_FALSE(cert.GetPathLengthConstraint().has_value());
}

TEST(HelpSslX509, match_peer_identity_empty_string)
{
    std::ifstream file("cert.pem");
    ASSERT_TRUE(file.good());
    auto cert = SslX509(std::move(file));

    EXPECT_FALSE(cert.MatchesPeerIdentity(""));
}
