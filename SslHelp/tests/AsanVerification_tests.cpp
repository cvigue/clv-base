...
{
#if CLV_ASAN_ENABLED
    // 1. Verify compile-time detection
    SUCCEED() << "ASAN is enabled at compile time.";

    // 2. Verify runtime poisoning
    // Allocate a small buffer
    volatile int *ptr = new int[10];
    delete[] ptr;

    // Check if the freed memory is poisoned
    // __asan_address_is_poisoned returns non-zero if the address is poisoned
...
#else
    // If not an ASAN build, we just inform.
    // If you want to ENFORCE ASAN, you can change this to FAIL().
