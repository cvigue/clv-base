...
// ============================================================================
// Type Info and poly_cast Tests
// ============================================================================

TEST(ImplCBMI, TypeReturnsCorrectTypeInfo)
{
    Duck duckA = DuckA();
    Duck duckB = DuckB();
    Duck duckC = DuckC(42);

    EXPECT_EQ(poly_type(duckA), typeid(DuckA));
    EXPECT_EQ(poly_type(duckB), typeid(DuckB));
    EXPECT_EQ(poly_type(duckC), typeid(DuckC));

    // Types should be different from each other
    EXPECT_NE(poly_type(duckA), poly_type(duckB));
    EXPECT_NE(poly_type(duckA), poly_type(duckC));
    EXPECT_NE(poly_type(duckB), poly_type(duckC));
}

TEST(ImplCBMI, TypeInfoAfterCopy)
{
    Duck d1 = DuckA();
    Duck d2 = d1; // Copy

    EXPECT_EQ(poly_type(d1), poly_type(d2));
    EXPECT_EQ(poly_type(d2), typeid(DuckA));
}

TEST(ImplCBMI, TypeInfoAfterMove)
{
    Duck d1 = DuckA();
    Duck d2 = std::move(d1);
...
TEST(ImplCBMI, TypeInfoAfterReassignment)
{
    Duck d1 = DuckA();
    EXPECT_EQ(poly_type(d1), typeid(DuckA));

    d1 = DuckB();
    EXPECT_EQ(poly_type(d1), typeid(DuckB));

    d1 = DuckC(99);
    EXPECT_EQ(poly_type(d1), typeid(DuckC));
}

TEST(ImplCBMI, TryCastSuccess)
{
    Duck duck = DuckC(42);

    const DuckC *ptr = poly_cast<DuckC>(duck);
    ASSERT_NE(ptr, nullptr);
    EXPECT_EQ(ptr->name(), "DuckC[42]");
}

TEST(ImplCBMI, TryCastFailure)
{
    Duck duck = DuckA();
    ...
    const DuckB *ptr2 = poly_cast<DuckB>(duck);
    EXPECT_EQ(ptr2, nullptr);
}

TEST(ImplCBMI, TryCastMutable)
{
    Duck duck = DuckC(42);

    DuckC *ptr = poly_cast<DuckC>(duck);
    ASSERT_NE(ptr, nullptr);
    EXPECT_EQ(ptr->name(), "DuckC[42]");

    // Verify we can call non-const methods on the result
    ptr->swim();
}

TEST(ImplCBMI, TryCastConst)
{
    const Duck duck = DuckC(42);
...
TEST(ImplCBMI, TryCastInVector)
{
    std::vector<Duck> ducks;
    ducks.push_back(DuckA());
    ducks.push_back(DuckB());
    ducks.push_back(DuckC(1));
    ducks.push_back(DuckC(2));
    ducks.push_back(DuckA());
    ...
    for (auto &duck : ducks)
    {
        if (poly_type(duck) == typeid(DuckC))
        {
            duckCCount++;
        }
        // Alternative using poly_cast
        if (auto *p = poly_cast<DuckC>(duck))
        {
            // Extract the number from "DuckC[N]"
            std::string name = p->name();
            auto start = name.find('[') + 1;
            auto end = name.find(']');
            duckCSum += std::stoi(name.substr(start, end - start));
        }
    }

    EXPECT_EQ(duckCCount, 2);
    EXPECT_EQ(duckCSum, 3); // 1 + 2
}

TEST(ImplCBMI, TypeInfoWithPointerWrapping)
{
    DuckA duckA;
    Duck duck = &duckA; // Wrap pointer

    // When wrapping a pointer, the type is the pointer type
    EXPECT_EQ(poly_type(duck), typeid(DuckA *));
    EXPECT_NE(poly_type(duck), typeid(DuckA));
}

TEST(ImplCBMI, TryCastWithPointerWrappingFails)
{
    DuckA duckA;
    Duck duck = &duckA;

    // poly_cast to the value type should fail - the erased type is DuckA*, not DuckA
    // Note: poly_cast<DuckA*> is not directly supported since Model<T*>::DelegateTo()
    // returns T& not T*&. Use poly_type() to check if it's a pointer-wrapped object.
    DuckA *wrongPtr = poly_cast<DuckA>(duck);
...
// Define a move-only Duck type using the MoveOnly tag
using MoveOnlyDuckPoly = Poly<DuckConcept, DuckDelegate>;

struct MoveOnlyDuck
{
    std::unique_ptr<int> data;

    MoveOnlyDuck(int val)
        : data(std::make_unique<int>(val))
    {
    }
    MoveOnlyDuck(MoveOnlyDuck &&) = default;
    MoveOnlyDuck &operator=(MoveOnlyDuck &&) = default;
    MoveOnlyDuck(const MoveOnlyDuck &) = delete;
    MoveOnlyDuck &operator=(const MoveOnlyDuck &) = delete;

    std::string name() const noexcept
    {
        return "MoveOnlyDuck[" + std::to_string(*data) + "]";
    }
    std::string walk() const noexcept
    {
        return "walks like a duck";
...
TEST(ImplCBMI, MoveOnlyPolyWorks)
{
    MoveOnlyDuckPoly duck = MoveOnlyDuck(42);
    EXPECT_EQ(duck.name(), "MoveOnlyDuck[42]");
    EXPECT_EQ(duck.walk(), "walks like a duck");
}

TEST(ImplCBMI, MoveOnlyPolyMove)
{
    MoveOnlyDuckPoly duck = MoveOnlyDuck(42);
    MoveOnlyDuckPoly moved = std::move(duck);
    EXPECT_EQ(moved.name(), "MoveOnlyDuck[42]");
}

TEST(ImplCBMI, MoveOnlyPolyMoveAssign)
{
    MoveOnlyDuckPoly d1 = MoveOnlyDuck(1);
    MoveOnlyDuckPoly d2 = MoveOnlyDuck(2);
    d1 = std::move(d2);
    EXPECT_EQ(d1.name(), "MoveOnlyDuck[2]");
}

TEST(ImplCBMI, MoveOnlyPolyWithCopyableType)
{
    // MoveOnlyPoly can also hold copyable types - they just can't be copied
    MoveOnlyDuckPoly duck = DuckA();
    EXPECT_EQ(duck.name(), "DuckA");

    MoveOnlyDuckPoly moved = std::move(duck);
    EXPECT_EQ(moved.name(), "DuckA");
}

// Note: The following would NOT compile (copy is disabled):
// MoveOnlyDuckPoly copy = duck;
