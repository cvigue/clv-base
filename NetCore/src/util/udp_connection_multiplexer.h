...
class UdpConnectionMultiplexer
{
  public:
        ...
        asio::io_context &io_ctx)
    {
        ...
        if (!key_opt)
        {
            if constexpr (requires(const KeyExtractorT &extractor, std::span<const uint8_t> packet) {
                              extractor.TryBuildConnectionlessResponse(packet);
                          })
            {
                auto response = key_extractor_.TryBuildConnectionlessResponse(data);
                if (response)
                {
            ...
            // Invalid or unroutable packet with no stateless response.
            co_return false;
        }
