...
/// Per-family constants for nftables rule construction
struct FamilyInfo
{
...
FamilyInfo GetFamilyInfo(std::uint8_t family)
{
    if (family == NFPROTO_IPV6)
        return {"clv_vpn_nat6", 16, 8, 24};
    return {"clv_vpn_nat", 4, 12, 16};
}
...
{
    const auto info = GetFamilyInfo(family);

    // First remove any existing table to make this idempotent
    if (TableExists(family))
        RemoveMasquerade(family);
