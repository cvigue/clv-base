...
// ==================== Overflow-propagation contract tests ====================
// These tests document the contract that callers rely on when they propagate
// NlaPut return values: offset is never advanced on failure, so the first
// failed check is sufficient to abort message construction safely.

TEST(NlaHelpersTest, OverflowPropagation_OffsetUnchangedAfterFailure)
{
    // Buffer fits exactly one u32 attribute.
    constexpr size_t attr_size = NLA_ALIGN(NLA_HDRLEN + sizeof(uint32_t));
    alignas(4) char buf[attr_size] = {};
    size_t offset = 0;
    uint32_t val = 0;

    ASSERT_TRUE(NlaPut(buf, offset, sizeof(buf), 1, &val, sizeof(val)));
    EXPECT_EQ(offset, attr_size);

    // Overflow: offset must not advance, so a caller checking the return can
    // detect the failure and abort without any partial write having occurred.
    const size_t offset_before = offset;
    EXPECT_FALSE(NlaPut(buf, offset, sizeof(buf), 2, &val, sizeof(val)));
    EXPECT_EQ(offset, offset_before);
}

TEST(NlaHelpersTest, OverflowPropagation_NestedChildOverflow)
{
    // Buffer holds the nested header plus exactly one child attribute.
    constexpr size_t child_size = NLA_ALIGN(NLA_HDRLEN + sizeof(uint32_t));
    constexpr size_t cap = NLA_HDRLEN + child_size;
    alignas(4) char buf[cap] = {};
    size_t offset = 0;
    uint32_t val = 0;

    auto *nested = NlaBeginNested(buf, offset, cap, 5);
    ASSERT_NE(nested, nullptr);
    EXPECT_EQ(offset, NLA_HDRLEN);

    EXPECT_TRUE(NlaPut(buf, offset, cap, 1, &val, sizeof(val)));

    // Second child does not fit; offset must be unchanged so the caller can
    // detect the failure immediately.
    const size_t offset_before = offset;
    EXPECT_FALSE(NlaPut(buf, offset, cap, 2, &val, sizeof(val)));
    EXPECT_EQ(offset, offset_before);
}

TEST(NlaHelpersTest, OverflowPropagation_MultipleAttrsLastOverflows)
{
    // Sequence of attrs where the last one just barely doesn't fit.
    constexpr size_t attr_size = NLA_ALIGN(NLA_HDRLEN + sizeof(uint32_t));
    constexpr size_t cap = attr_size * 3; // fits 3, attempt 4
    alignas(4) char buf[cap] = {};
    size_t offset = 0;
    uint32_t val = 0;

    EXPECT_TRUE(NlaPut(buf, offset, cap, 1, &val, sizeof(val)));
    EXPECT_TRUE(NlaPut(buf, offset, cap, 2, &val, sizeof(val)));
    EXPECT_TRUE(NlaPut(buf, offset, cap, 3, &val, sizeof(val)));
    EXPECT_EQ(offset, cap); // exactly full

    const size_t offset_before = offset;
    EXPECT_FALSE(NlaPut(buf, offset, cap, 4, &val, sizeof(val)));
    EXPECT_EQ(offset, offset_before);
}
